...
class MCCMNCRecordList(RootModel):
    root: List[MCCMNCRecord]
...
class MCCMNCFetcher:
    ...
    def json_file() -> None:
        ...
        mcc_mnc_record_list = MCCMNCRecordList(mcc_mnc_records)

        with open("mcc-mnc-table.json", "w") as f:
            json.dump(mcc_mnc_record_list.model_dump(), f, indent=2)
...
def create_dart_file():
    mcc_mcc_map = {}

    with open("mcc-mnc-table.json", "r") as file:
        # Load the JSON data
        data = json.load(file)
        ...
        for entry in data:
            ...
            if entry['mnc']:
                mnc = entry['mnc']
            if entry['network']:
                operator = unidecode(entry['network'])
